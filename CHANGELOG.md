...
### Added
- Cooking sessions: manually start and stop a recording session via flow
  actions, with an optional note at start and any number of notes added
  during the cook.
- `cooking_session_active` (boolean) and `cooking_session_name` (string)
  capabilities so the active session is visible in the device UI.
- Flow triggers `session_started` and `session_ended`, the latter with
  tokens for duration, sample count, peak/avg pit temperature, and the
  written file path.
- Flow actions `start_session`, `stop_session`, and `add_session_note`.
- Session data is recorded once per poll and persisted on stop to
  `/userdata/sessions/<timestamp>_<slug>.json` along with an `index.json`
  catalog. Active sessions are also periodically snapshotted to per-device
  store so an in-progress cook survives app restarts.
- Session JSON files are reachable over HTTPS at
  `https://<homey>/app/com.bbqguru.cyberq/userdata/sessions/...`.
